<?php
	...
	$id = $DBH->quote($_GET['product_id']);

	$STH = $DBH->query(
		"SELECT Product.Product_Id AS product_id, Product.Name AS product_name, Description, Details, Featured, Category_Id, Price, Quantity
		   FROM Inventory 
		...
		<form enctype="multipart/form-data" class="form-horizontal" role="form" method="POST" data-toggle="validator">
			<div class="col-md-6">
				...
				<div class="form-group">
					...
					<div class="col-md-8">
						<textarea rows="5" class="form-control" name="description" placeholder="Description" required>
							<?= $product['Description'] ?>
						</textarea>
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-md-8">
						<textarea rows="5" class="form-control" name="details" placeholder="Details" required>
							<?= $product['Details'] ?>
						</textarea>
					</div>
				</div>
			...
			<div class="col-md-6">
				...
				?>
						<div class="form-group">
							<label for="image1" class="col-md-2 control-label">Image
								<?= $i ?>
							</label>
							<div class="col-md-5" style="outline: solid 1px">
								<img src="<?= $images[$i - 1]['Image_Url'] ?>" />
							</div>
							<div class="col-md-5">
								<div class="checkbox">
									<label>
										<input type="checkbox" name="deleteimage[]" value="<?= $images[$i - 1]['Product_Image_Id'] ?>"><b>Delete?</b></label>
								</div>
							</div>
						</div>
